...
if st.session_state.rol == "ADMIN":
    ...
    t1, t2 = st.tabs(["📋 Control de Clientes", "👤 Usuarios Adicionales"])
    
    with t1:
        st.subheader("Estado de Entidades (10,000+ Empresas)")
        clientes = []
        for r, info in st.session_state.db['empresas'].items():
            clientes.append({"RIF": r, "Empresa": info['nombre'], "Vencimiento": info['vence'], "Estado": info['estado']})
        st.dataframe(pd.DataFrame(clientes), use_container_width=True)
        
    with t2:
        with st.form("new_user"):
            st.subheader("Vincular Múltiples Usuarios a una Empresa")
            rif_target = st.selectbox("Seleccione Empresa", list(st.session_state.db['empresas'].keys()))
            u_mail = st.text_input("Correo del Nuevo Usuario")
            u_pass = st.text_input("Clave Temporal", type="password")
            if st.form_submit_button("Habilitar Usuario"):
                st.session_state.db['usuarios'][u_mail] = {'pass': make_hashes(u_pass), 'rol': 'CLIENTE', 'rif': rif_target}
                st.success(f"Usuario {u_mail} vinculado a {rif_target}")
